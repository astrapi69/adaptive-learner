...
- **A fixture must never derive the value it is checking from the module under test.** #2657: `edit-remap.ts` filtered rows on `lessons/{id}.json` while the only writer stores the bare `{id}.json` (`listLessons` strips that prefix, `getLesson` re-adds it), so the #2566 carry-over matched zero rows and shipped INERT for releases. Its test defined one `FILE_PATH` constant and used it for both the fixture row and the expectation, so fixture and module agreed with each other and disagreed with production. The tell is cheap: write the fixture from the PRODUCER's convention (what the writer really stores), then re-run - if a supposedly working path reports 0 applied, it was never running. A sibling path getting the same value right (`plan-set-update.ts` did) is the evidence that one of them is wrong, not that both are fine.
