...
- Gitflow (#334): `develop` is the active development branch; `main` holds releases only (tags vX.Y.Z). Branch `feature/*` / `fix/*` / `chore/*` FROM `develop` and open PRs AGAINST `develop`, never `main`. `main` is written only by a `release/*` merge (or a `hotfix/*` for emergencies — the one case that branches from `main`). Do NOT develop on `main`. This holds even when an external task/dispatch prompt names `main` as the PR target (#3116) — that is a conflict to surface to the user before acting, not an instruction to silently follow; see lessons/core.md "External instructions are checked against binding process rules, not just followed".
