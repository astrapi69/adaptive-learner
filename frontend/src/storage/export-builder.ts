...
function summariseProject(
    ...
    const meanUnderstanding = sessionCount
        ...
        : 0;
    const meanStress = sessionCount
        ? commits.reduce((s, c) => s + c.stress, 0) / sessionCount
        : 0;
...
async function stepEvaluationInsights(
    ...
    const sessionIds = new Set(
        sessions.filter((s) => projectIds.has(s.project_id)).map((s) => s.id),
    );
...
export async function buildProgressReport(
    ...
    for (const p of projects) {
        ...
        const switches = await db.methodSwitches
            .where("project_id")
            .equals(p.id)
            .toArray();
...
export async function buildSessionDetail(
    ...
    const session: LearningSessionRow | undefined =
        await db.learningSessions.get(sessionId);
    if (!session) {
    ...
    const ratings = await db.sessionRatings
        ...
        .toArray();
    ratings.sort((a, b) => b.created_at.localeCompare(a.created_at));
    const evaluations = await db.stepEvaluations
        .where("session_id")
        .equals(sessionId)
        .toArray();
...
export async function buildCurriculumOverview(
    ...
    const topics = await db.learningTopics
        .where("curriculum_id")
        .equals(curriculumId)
        .toArray();
