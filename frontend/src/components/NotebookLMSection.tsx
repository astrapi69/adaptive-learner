...
export default function NotebookLMSection({projectId}: NotebookLMSectionProps) {
            ...
            const msg =
                err instanceof ApiError
                    ...
                    : t(
                          ...
                          "Could not load study questions.",
                      );
            notify.error(msg);
            setQuestions([]);
        }
    ...
    const generateQuestions = async () => {
        setGenerating(true);
        try {
            const rows = await getStorage().notebooklm.generateFromProject(
                projectId,
            );
            if (rows.length === 0) {
                notify.info(
                ...
                notify.success(
                    t(
                        ...
                        "Generated {n} new study question(s).",
            ...
            const msg =
                err instanceof ApiError
                    ...
                    : t(
                          ...
                          "Could not generate study questions.",
                      );
            notify.error(msg);
        } finally {
            setGenerating(false);
    ...
    const removeQuestion = async (q: StudyQuestion) => {
        ...
        try {
            ...
            setQuestions((prev) =>
                prev ? prev.filter((x) => x.id !== q.id) : prev,
            );
        } catch (err) {
    ...
    const downloadStudyGuide = async () => {
        ...
        try {
            ...
            URL.revokeObjectURL(url);
            notify.success(
                t("notebooklm.guide_ready", "Study guide downloaded."),
            );
        } catch (err) {
            const msg =
                err instanceof ApiError
    ...
    return (
        <section
            ...
            data-testid="notebooklm-section"
        >
            ...
            {questions === null ? (
                ...
                <ul
                    className="notebooklm-question-list"
                    data-testid="notebooklm-question-list"
                >
                    {questions.map((q) => (
                        <li
                            key={q.id}
                        ...
                        >
                            <div className="notebooklm-question__meta">
                                ...
                                {q.topic && (
                                    <span className="notebooklm-question__topic">
                                        {q.topic}
                                    </span>
                                )}
                                <span className="notebooklm-question__type">
                                    {q.question_type}
                                </span>
                            </div>
