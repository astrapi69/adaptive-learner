...
export default function DailyMissionsCard({ userId }: DailyMissionsCardProps) {
  ...
  const [enabled, setEnabled] = useState<boolean>(() => readMissionPrefs().enabled);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const allComplete = result.missions.length > 0 && result.missions.every((m) => m.completed);
  ...
  const allDone = missions !== null && missions.length > 0 && missions.every((m) => m.completed);
  ...
  return (
    <div className="daily-missions" data-testid="daily-missions">
      ...
      {missions === null ? (
        ...
        <ul className="mission-list">
          {missions.map((m) => {
            ...
            const missionTitle = t(m.template.title_key, m.template_id);
            ...
            return (
              ...
              >
                ...
                <div className="mission-body">
                  <span className="mission-title">{missionTitle}</span>
                  <div
                    className="mission-progressbar"
                    role="progressbar"
                    aria-label={`${missionTitle}: ${m.progress} / ${m.target}`}
                    aria-valuenow={m.progress}
                    aria-valuemin={0}
                    aria-valuemax={m.target}
                  >
                    <div className="mission-progressfill" style={{ width: `${pct}%` }} />
                    <span
                ...
                <span className="mission-xp">+{m.template.xp_reward} XP</span>
                {m.completed && (
                  <CheckCircle2 className="mission-done-icon" size={18} aria-hidden="true" />
                )}
              </li>
            );
          })}
      ...
      {allDone && (
        <p className="daily-missions-alldone" data-testid="daily-missions-alldone">
