...
} from "../../components/import/GenerateExercisesButton";
...
export default function ImportDetail({
  ...
  // AIX-02 (#826) — exercises generated by the AI from a theory-only
  // lesson's prose; previewed below the analysis and folded into the
  // saved set.
  ...
  // AIX-02 (#826) — derive the deterministic base lesson from the analysis
  // (same builder the Save modal uses) to decide whether the "Generate
  // exercises" button applies (a theory-only lesson: >= 1 theory step,
  // 0 exercises) and to feed the theory prose to the AI.
  let theorySteps: TheoryStep[] = [];
  let isTheoryOnly = false;
  if (analysis) {
    try {
      const baseLesson = generateLessonFromAnalysis(analysis, {
        id: `analysis-${slugify(detail.id)}`,
        labels: analysisLessonLabels(t),
      });
      const baseSummary = summarizeGeneratedLesson(baseLesson);
      isTheoryOnly =
        baseSummary.theorySteps >= 1 && baseSummary.exercises === 0;
      theorySteps = baseLesson.steps
        .filter((step) => step.type === "theory")
        .map((step) => ({ id: step.id, title: step.title, body: step.body }));
    } catch {
      /* a malformed analysis just means no generate-exercises offer */
    }
  }
  const showGenerate = isTheoryOnly || generatedExercises.length > 0;
