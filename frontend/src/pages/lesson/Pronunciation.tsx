...
export default function PronunciationPage() {
  ...
  // Issue 4 / feature-strategy — generating a phrase fires an AI
      ...
      {verdict && (
        <section
          className={
            ...
            (verdict.matches ? "pronunciation-verdict--match" : "pronunciation-verdict--miss")
          }
          data-testid="pronunciation-verdict"
          data-match={verdict.matches ? "true" : "false"}
        >
          <p className="pronunciation-verdict__score">
            <strong>{Math.round(verdict.score * 100)}%</strong>{" "}
            {verdict.matches
              ? t("pronunciation.match", "Match")
              : t("pronunciation.miss", "Keep practising")}
          </p>
          <p className="pronunciation-verdict__feedback">{verdict.feedback}</p>
          {verdict.missed_sounds.length > 0 && (
            <p className="pronunciation-verdict__sounds" data-testid="pronunciation-missed-sounds">
              {t("pronunciation.focus_on", "Focus on:")} {verdict.missed_sounds.join(", ")}
            </p>
          )}
        </section>
      )}
