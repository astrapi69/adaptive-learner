/**
 * BUG P1 — single two-phase button in the REVIEW (SRS) flow.
 *
 * Regression-pin for the "two buttons visible" report. The review
 * session used to render each exercise UNCONTROLLED (so the renderer
 * showed its own internal "Antwort prüfen" submit) AND its own
 * "Weiter" nav button — two buttons at once during every exercise
 * step. This pins the controlled two-phase flow: no internal submit,
 * exactly one shared button (Check -> Weiter) per exercise step.
 *
 * Mocks only useReviewLesson + getStorage; the real ExerciseDispatcher
 * + real renderers run.
 */
...
function mountExercise(exercise: ContentLessonExercise) {
  ...
  return render(
    <MemoryRouter initialEntries={["/review/language-fr-a1"]}>
      <Routes>
...
describe("Review BUG P1: one two-phase button, no internal submit", () => {
  for (const { name, exercise } of CASES) {
    ...
    it(`${name}: exactly one flow button, disabled "Check", no "Next" before answering`, () => {
      mountExercise(exercise);
      const checkBtn = screen.getByTestId("review-check");
