 ...
 *     indeterminate progress bar + phase label, button disabled)
...
const h = vi.hoisted(() => {
    ...
    return {
        ...
        saveAnalysisMock: vi.fn(
            async (_id: string, body: {analysis_result: unknown}) => ({
                ...detail,
...
describe("ImportDetail analysis loading indicator", () => {
    it("shows the loading state when analysis starts", async () => {
        ...
        expect(
            screen.getByTestId("analysis-progress-label").textContent,
        ).toContain("Step 1/3");
        expect(screen.getByTestId("analysis-cancel")).toBeInTheDocument();
    });
    ...
    it("cancels and returns to the pre-analysis state", async () => {
        ...
        await waitFor(() =>
            expect(
                screen.queryByTestId("analysis-progress"),
    ...
    it("renders a friendly inline error when analysis fails", async () => {
        ...
        const err = await screen.findByTestId("analysis-error");
