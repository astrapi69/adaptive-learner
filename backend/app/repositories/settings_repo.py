...
class SqlAlchemySettingsRepository(SettingsRepository):
    ...
    def get_or_create_settings(self, user: User) -> UserSettings:
        ...
        Race-safe: a concurrent first-access that loses the
        ``unique(user_id)`` insert rolls back and re-reads the committed
    ...
    def persist(self, settings: UserSettings) -> None:
        """Commit the transaction and refresh the settings row."""
        self._db.commit()
        self._db.refresh(settings)
    ...
    def upsert_api_key_backup(
        ...
        Marks the stored key as working and commits.
        """
