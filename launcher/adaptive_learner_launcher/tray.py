...
# (gir1.2-ayatanaappindicator3-0.1) is the reliable path on Ubuntu (#1003).
...
class TrayController:
    ...
    def start(self) -> bool:
        ...
        available, or the platform tray never appeared (e.g. the Linux
        AppIndicator GIR typelib is not installed). In every False case the
        caller falls back to closing the window, so the window is never
        ...
        that is unimplemented on the Linux GTK/AppIndicator backend and was
        the cause of the tray silently failing on Ubuntu (#1003).
        """
        if not HAS_TRAY:
            logger.info(
        ...
        self._thread = threading.Thread(target=_run_loop, name="al-tray", daemon=True)
        self._thread.start()
