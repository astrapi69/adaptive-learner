...
## Archived 2026-05-30 (v1.45.1 — stale-entry reconciliation)

Both items were already fixed in-tree but never archived from
``backlog.md`` (same backlog-staleness pattern that the
ROADMAP refresh corrected). Verified done during the v1.45.0 →
v1.45.1 review and closed here with their shipping commits.

- [x] **BL-23**: ``get_or_create_settings`` race condition —
  concurrent first-access (React 18 strict-mode double-effect
  or parallel ``GET /api/settings/{id}``) used to pass the
  ``user.settings is None`` check twice, both INSERT, and the
  loser hit the ``unique=True`` constraint on ``user_id`` and
  bubbled up as HTTP 500. Fixed in commit ``bdb0696``
  (``fix(settings): survive concurrent first-access in
  get_or_create_settings``): the loser catches ``IntegrityError``,
  rolls back the doomed insert, and re-reads the winner's row.
  Regression pin
  ``test_get_or_create_settings_survives_concurrent_first_access``
  (``backend/tests/test_settings_router.py``) simulates the race
  and asserts exactly one row survives — green.
- [x] **BL-24**: E2E ``page.route`` GET matcher for
  ``/api/imports/{id}`` — the original spec could only
  ``waitForRequest`` on the analyze POST, never assert the
  detail-page cards, because ``page.route`` on the GET never
  fired. Root-caused in commit ``abc0d94``
  (``test(e2e): unblock GET /api/imports/{id} interception via
  serviceWorkers: 'block'``): the dev Service Worker's
  ``NetworkFirst`` rule intercepted ``GET /api/*`` before
  Playwright could. ``test.use({serviceWorkers: "block"})``
  scoped to ``conversation-import.spec.ts`` disables SW
  registration so the GET mock fires and the spec now asserts
  the ``analysis-results`` card renders.
