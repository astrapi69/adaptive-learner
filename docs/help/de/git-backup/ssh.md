...
AdaptiveLearner erzeugt **einen Schlüssel pro Installation** (nicht pro Buch). Derselbe Schlüssel funktioniert für alle SSH-Remotes.

## Schlüssel in AdaptiveLearner erzeugen

1. **Einstellungen > Allgemein** öffnen.
2. Abschnitt **SSH-Schlüssel für Git** finden.
3. Optional: einen **Kommentar** eintragen (z. B. `adaptive-learner-aster-laptop`). Dieser erscheint im UI des Git-Hosts als Label.
4. **Schlüssel erzeugen** klicken.

AdaptiveLearner generiert ein Ed25519-Schlüsselpaar nach OpenSSH-Format. Der private Teil wird unter `config/ssh/id_ed25519` mit Rechten `0600` abgelegt; der öffentliche unter `config/ssh/id_ed25519.pub` mit `0644`.
...
2. **Title**: aussagekräftig, z. B. „AdaptiveLearner (Arbeit)".
...
Beim nächsten Push/Pull greift AdaptiveLearner automatisch auf den SSH-Schlüssel zu — keine weitere Eingabe nötig.
...
- **Rechte 0600 sind Pflicht.** OpenSSH verweigert Schlüssel mit offeneren Rechten. AdaptiveLearner setzt die Rechte automatisch.
...
1. SSH-Schlüssel in AdaptiveLearner erzeugen (siehe oben).
2. Öffentlichen Teil beim Host hinterlegen.
3. Im Buch-Dialog **Remote bearbeiten** → URL auf SSH-Form ändern.
4. Ersten Push machen — AdaptiveLearner nutzt nun SSH.

Der alte PAT bleibt verschlüsselt gespeichert, falls du zurückwechseln willst. Zum Entfernen: in AdaptiveLearner **Remote löschen**, dann neu mit nur der SSH-URL (ohne PAT) konfigurieren.
...
Erster Kontakt mit einem Host. AdaptiveLearner akzeptiert unbekannte Hosts einmalig (`StrictHostKeyChecking=accept-new`) und pinnt den Fingerprint für alle weiteren Verbindungen. Wenn die Fehlermeldung bei einem bekannten Host auftritt, könnte ein Man-in-the-Middle im Spiel sein — nicht ignorieren.

**SSH-Schlüssel verloren.**
Privaten Schlüssel nicht mehr auffindbar (AdaptiveLearner neu installiert, Home-Verzeichnis gelöscht, etc.): in **Einstellungen > SSH-Schlüssel** neuen Schlüssel erzeugen, dabei Option **überschreiben** bestätigen. Alten öffentlichen Schlüssel beim Host entfernen, neuen hinterlegen.
