...
`lesson_count`, `path`, `domain`, `tags`, `book`) βρίσκεται στην
αναφορά του engine:
...
| `ext:al-dictation` | Ηχητική υπαγόρευση (άκου, μετά μετάγραψε) | `audio` (ένα κλιπ `assets/`) + `accept[]` (ανεκτική αντιστοίχιση μεταγραφής) | #1881 (πέμπτη υιοθέτηση) |
...
| Ήχος / βίντεο / μεταφόρτωση αρχείου | Αποθήκευση + υποδομή· συγκρούεται με το offline-first. |
