...
`version`、`lesson_count`、`path`、`domain`、`tags`、`book`）は、
エンジンのリファレンスにあります：
...
| `ext:al-dictation` | 音声のディクテーション（聞いてから書き取る） | `audio`（`assets/` のクリップ）+ `accept[]`（寛容な書き起こしマッチング） | #1881（5 番目の採用） |
...
| 音声 / 動画 / ファイルアップロード | ストレージ + インフラ。オフラインファーストと衝突する。 |
