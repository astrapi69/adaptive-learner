...
**PR-PFLICHT (Regel in `.claude/rules/ai-workflow/pr-policy.md`):** Jede gepushte Code-Änderung öffnet einen PR gegen `develop` - IMMER, unabhängig davon, ob der Auftrag es explizit verlangt. "Kein PR, nicht angefordert" ist keine gültige Abschluss-Meldung; ein gepushter Branch ohne PR ist unfertige Arbeit. Ausnahmen nur: Release-Sperre und reine Analyse-/Status-/Doku-Frage-Aufträge, die nichts committen. Ein explizites "nur pushen, kein PR" wird respektiert; das Fehlen einer expliziten Aufforderung ist kein Grund, den PR zu überspringen.
**TESTPLAN-PFLICHT (Regel in `.claude/rules/ai-workflow/testplan-policy.md`):** Jeder PR mit nutzersichtbarer Funktionalität (neue Buttons, neue Wizard-Schritte, neue Übungstypen, geänderte Nutzerabläufe) aktualisiert den manuellen Testplan (`docs/manual-tests/testplan-adaptive-learner.md` + `testplan-adaptive-learner-en.md`, beide Sprachfassungen synchron) im SELBEN PR - oder hinterlässt, wenn das den PR-Scope sprengen würde, einen direkt referenzierten Follow-up-Kommentar auf #1087 mit den ausstehenden Testplan-Ergänzungen. "Testplan-Update wurde nicht explizit gefordert" ist keine gültige Begründung, es wegzulassen - dritte Regel der PFLICHT-Familie neben GITHUB-ISSUE-PFLICHT und PR-PFLICHT. Ausnahmen: reine interne Refactorings ohne Verhaltensänderung (z. B. die #1450-Datei-Splits), reine Infrastruktur-/CI-Änderungen, reine Doku-Fixes, Bugfixes ohne neuen Nutzerpfad (kein neuer Testfall nötig; ein bestehender Testfall wird aber korrigiert, falls er die Regression beschrieb).
