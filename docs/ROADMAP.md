...
Current state: **v1.48.0 released 2026-06-01 (Phase 64 — Community Sharing UX + Smart Lesson Organization: a four-step share wizard with a smart placement engine (auto tree path + auto-numbered filename + new-set detection + content auto-detection), advisory lesson-level duplicate/variation/supplement detection, optional author credit (contributed_by/at, remembered locally), a local contribution history + "Community Contributor" recognition, and encouraging Missing-Lessons gap suggestions; content schema 1.2 -> 1.3 additive. Merges Smart Next-Step Suggestions after lesson completion (adaptive / review / set-complete / resume awareness) and adds the EXP-021 Lesson-Creator exploration. Full i18n in 8 languages).** Prior: **v1.47.0 (Phase 63 — Lesson Flow Control: pause/abandon/resume + 30s autosave + auto-resume, paused-lessons widget, lesson splitter, retention sweep; Word Tiles @dnd-kit touch reorder, mobile overflow fixes, backend CSP pass).** Earlier: **v1.46.0 (Phase 62 — EXP-018 Exercise Direction) and v1.45.x (Phase 61 quality sweep + docs-sync).** 1064 (+1 skipped) backend + 997 plugin + 2816 Vitest = 4877 tests green (+1 skipped). Playwright smoke spec files run separately; the Dexie-mode release gate runs via `make test-dexie-smoke`.
