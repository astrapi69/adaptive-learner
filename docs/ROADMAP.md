...
Current state: **v1.47.0 released 2026-06-01 (Phase 63 — Lesson Flow Control: lessons pause/abandon/resume with 30s autosave + auto-resume on tab return, a Dashboard paused-lessons widget, a lesson splitter for oversized imports (configurable 5-20 exercises, cross-language parity test) and a paused-lesson retention sweep; folds in Word Tiles touch-capable drag-to-reorder (@dnd-kit), mobile horizontal-scroll fixes (3 overflow sources + regression spec), lower-friction community sharing, and a backend CSP/security-header pass (Phase 61 audit P3)).** Prior: **v1.46.0 (Phase 62 — EXP-018 Exercise Direction: every exercise carries a receptive/productive direction, SRS tracks mastery per direction (Alembic 0023 + Dexie v23), direction-aware renderers + review weighting + adaptive direction-strategy + Settings control + dashboard split; folds in a P0 analysis-to-lesson Save fix and the lesson-content migration into adaptive-learner-content).** Earlier: **v1.45.1 (patch / docs-sync) and v1.45.0 (Phase 61 — quality sweep).** 1064 (+1 skipped) backend + 997 plugin + 2722 Vitest = 4783 tests green (+1 skipped). Playwright smoke spec files run separately; the Dexie-mode release gate runs via `make test-dexie-smoke`.
