...
## 9. EXP-052 Scheibe 1: Review auf der Runner-Hülle (2026-09-23, 12:45 bis 15:30)

- Original prompt: "go", dann die Ergänzung zur Sperre beantworteter Schritte ("Die Scheibe-1-Liste nennt unter 'Verhalten, das neu dazukommt' nur Neuausrichtung und Hinweis-Löschen. Die Sperre gehört dazu").
- Optimized prompt: Scheibe als Agentin im Worktree von origin/develop mit vollständiger Bausteinliste, Testid-Liste, Baseline-Verfahren (Löschen-dann-Resync) und FeatureShot-Regeln; Zwischenergänzungen per SendMessage.
- Goal: `Review.tsx` unter 120 Zeilen auf `LessonRunner` mit `REVIEW_POLICY`; Hülle komponiert Kopf, Balken, Schritt, Fuß, Statusbildschirme, Zusammenfassung, Enter, Neuausrichtung, Hinweis-Löschen, Sperre; Baselines und FeatureShots im selben PR; Testplan DE + EN.
