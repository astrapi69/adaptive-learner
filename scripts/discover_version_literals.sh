...
#
# Closes the "closed-set discovery" gap identified by the Phase
# 40 release-automation audit: scripts/verify_version_pins.sh's
...
# Per-plugin __init__.py and plugin.py. Three of the ten plugins
# (gamification, anki, notebooklm) carry a static ``__version__``
# literal that sync-versions propagates; the other seven derive
# via ``importlib.metadata.version()`` with a ``0.0.0+unknown``
# sentinel fallback. Both shapes are author-managed within the
# plugin tree and don't need discovery flagging.
while IFS= read -r p; do
    KNOWN_FILES+=("${p#"$ROOT/"}")
done < <(find "$ROOT/plugins" -path '*/adaptive_learner_*/plugin.py' -type f 2>/dev/null)
while IFS= read -r p; do
    KNOWN_FILES+=("${p#"$ROOT/"}")
done < <(find "$ROOT/plugins" -path '*/adaptive_learner_*/__init__.py' -type f 2>/dev/null)
