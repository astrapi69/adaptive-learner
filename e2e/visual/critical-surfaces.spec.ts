...
for (const surface of SURFACE_NAMES) {
    for (const viewport of VIEWPORT_NAMES) {
        test(`${surface} renders correctly at ${viewport}`, async ({page}) => {
            ...
            // TEMP-DEBUG #2703 - remove before merge. review-session-*.png
            // wasn't re-generated by the previous --update-snapshots=changed
            // run even though assertSurfaceStillReady confirmed review-page
            // was visible right before the shot. Force an UNCONDITIONAL
            // capture to a name with no existing baseline (so Playwright
            // always writes it, regardless of any diff-vs-threshold
            // decision) to see the actual pixels.
            if (surface === "review-session") {
                await expect(page).toHaveScreenshot(`DEBUG2703-${surface}-${viewport}.png`);
            }
        });
    }
}
